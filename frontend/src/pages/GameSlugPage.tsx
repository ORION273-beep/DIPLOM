...
export default function GameSlugPage() {
  ...
  if (loading) return <PageLoader label="Загрузка игры..." />;

  if (notFoundState || !game) {
    return (
      <div className="container mx-auto px-4 py-20 text-center">
        <h1 className="text-4xl font-bold text-error-primary">Игра не найдена</h1>
        <Button href="/games" color="primary" className="mt-8">
          ← Все игры
        </Button>
      </div>
    );
  ...
  return (
    <div className="container mx-auto px-4 py-8 lg:py-12">
      <Link
        to="/games"
        className="mb-6 inline-flex items-center gap-2 text-sm text-tertiary transition-colors hover:text-brand-secondary"
      >
      ...
      <div className="relative mb-12 overflow-hidden rounded-3xl bg-gradient-to-r from-secondary to-secondary_hover p-8 lg:p-12">
        <div className="lg:grid lg:grid-cols-2 lg:gap-12">
          ...
          <div className="space-y-6 lg:pt-4">
            ...
            <p className="text-lg leading-relaxed text-secondary lg:text-xl">{game.description}</p>
      ...
      <section>
        <div className="mb-8 flex items-center justify-between">
          ...
          <span className="text-sm text-quaternary">{products.length} товаров</span>
        </div>
      ...
      {products.length > 0 && (
        <div className="mt-12 rounded-2xl bg-gradient-to-r from-brand-600/10 to-brand-500/5 p-8 text-center lg:p-12">
          <h3 className="mb-3 text-xl font-bold text-brand-secondary lg:text-2xl">Не нашли нужное?</h3>
          <Button
            href="/catalog"
            color="primary"
            iconTrailing={
