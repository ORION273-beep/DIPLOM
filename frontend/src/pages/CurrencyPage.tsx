...
export default function CurrencyPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const { products: items, error: e } = await fetchProducts({ currency: '1', limit: '100' });
      if (cancelled) return;
      setProducts(items);
      setError(e);
      setLoading(false);
    ...
    <section>
      ...
      <div className="container mx-auto px-4 pb-10">
            ...
            {orderedGroups.map(([group, items]) => (
              <div key={group}>
                <div className="mb-4 flex items-center justify-between">
                  <h2 className="text-2xl font-bold text-primary">{gameLabels[group] ?? group}</h2>
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          <Button href="/games" color="primary">
            Открыть игры
          </Button>
          <Button href="/catalog" color="secondary">
            Весь каталог
          </Button>
        </div>
      </div>
